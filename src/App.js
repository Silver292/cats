  ...
  return (
    <>
      ...
      <div className={`fixed inset-0 bg-white w-56 p-3 pt-28 transition-transform ${showBasket ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        {basket.map(item => <div key={item.id} className='grid grid-cols-2 my-3'>
          <div className='flex items-center'>
            <img className='inline object-cover w-6 h-6 rounded-full mr-2' src={item.imageUrl} alt={item.name} />
            <p>{item.name}</p>
          </div>
          <p className='text-right'>£{item.price}</p>
          <svg xmlns='http://www.w3.org/2000/svg' className='h-6 w-6 col-start-2 text-red-600 border border-red-300 rounded justify-self-end mt-2' fill='none' viewBox='0 0 24 24' stroke='currentColor' onClick={() => removeFromBasket(item)}>
        ...
        {basket.length ? (
          <div className='grid grid-cols-2 my-3'>
            <h3>Total:</h3>
            <h3 className='text-right'>£{basket.reduce((prev, curr) => prev + parseFloat(curr.price), 0.0).toFixed(2)}</h3>
          </div>)
          :
          null}
      </div>
